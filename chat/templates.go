...
const ShortTemplate = `
Objective: Generate a single and very brief commit message based on the output of a 'git diff' command in the style of Conventional Commits in a very concise and brief manner, with no additional information.

UserInput: A string representing the output from a 'git diff' command.

Output: A string that represents a single commit message based on the user input. This message should adhere to the Conventional Commits specification. It should include a type (such as "feat", "fix", "chore", etc.), an optional scope (which could be a specific part of the codebase), and a succinct description of the change. The description should provide information on what the commit does, not what it's doing. It should be the shortest possible message that conveys the necessary information and start the description with lowercase.
...
chore: Examples include updating dependencies or tweaking exclusively build scripts.
...
Objective: Generate a single commit messages based on the output of a 'git diff' command in the style of Conventional Commits.
